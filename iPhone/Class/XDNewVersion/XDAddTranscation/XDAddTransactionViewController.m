...
@property (weak, nonatomic) IBOutlet UILabel *categroyLbl;
...
-(Category *)otherCategory_income{
    if (!_otherCategory_income) {
        NSLog(@"category = %@",[[XDDataManager shareManager]getObjectsFromTable:@"Category" predicate:[NSPredicate predicateWithFormat:@"isDefault = 1 and categoryType = %@",@"INCOME"] sortDescriptors:nil]);
...
//    }
    ...
    [self.categroyLbl addGestureRecognizer:tap];
}
...
-(void)editTransactionConfig{
    ...
    self.datePicker.date = editTransaction.dateTime;
...
//        self.moneyTextField.enabled = NO;
        ...
        if (editTransaction.category) {
            ...
            self.categroyLbl.text = _selectedCategory.categoryName;
        }else{
...
//        [self itemClick:self.expenseBtn];
        ...
        if (editTransaction.expenseAccount && editTransaction.incomeAccount) {
...
-(void)newTransactionConfig{
    ...
    self.categroyLbl.text = _selectedCategory.categoryName;
    self.datePicker.date = calSelectedDate;
    self.datePickerLabel.text = [self returnInitDate:calSelectedDate];
    _fromAccount = nil;
    _toAccount = nil;
    _repeatStr = @"Never";
...
-(void)initLblName{
    ...
    self.categroyLbl.text = NSLocalizedString(@"VC_Category", nil);
...
-(void)saveEditTransaction{
    ...
    if (self.titleTextField.text.length > 0) {
        Payee* payee = [[[XDDataManager shareManager] getObjectsFromTable:@"Payee" predicate:[NSPredicate predicateWithFormat:@"state = %@ and name = %@",@"1",self.titleTextField.text] sortDescriptors:nil] lastObject];
        if (!payee) {
...
//        NSDate* date = [calendar dateFromComponents:startComponents];
    ...
    if (self.titleTextField.text.length > 0) {
        Payee* payee = [[[XDDataManager shareManager] getObjectsFromTable:@"Payee" predicate:[NSPredicate predicateWithFormat:@"state contains[c] %@ and name = %@",@"1",self.titleTextField.text] sortDescriptors:nil] lastObject];
        if (!payee) {
            Payee* newPay = [[XDDataManager shareManager]insertObjectToTable:@"Payee"];
            newPay.dateTime = newPay.updatedTime = [NSDate date];
            newPay.state = @"1";
...
-(void)returnSelectedPayee:(Payee *)payee{
    ...
    if (payee.category) {
        ...
        _splitCategoryArr = nil;
        self.categroyLbl.text = [[_selectedCategory.categoryName componentsSeparatedByString:@": "]lastObject];
        [self.categoryIconBtn setBackgroundImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@_white.png",[[_selectedCategory.iconName componentsSeparatedByString:@"."]firstObject]]] forState:UIControlStateNormal];
...
//    NSLog(@"%@",category);
    ...
    _splitCategoryArr = nil;
    self.categroyLbl.text = [[category.categoryName componentsSeparatedByString:@": "]lastObject];
