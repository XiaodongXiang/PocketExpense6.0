...
//            double num = [self getChartAmountMax:account];
        ...
        for (int i = 0 ; i < dateMuArr.count ; i++) {
            ...
            [dataMuArr addObject:@([self getAmountWithTransactionArr:array chartType:chartType account:account monthEndDate:[self getMonthEndDate:startDate]])];
        }
...
//    [calendar setTimeZone:[NSTimeZone timeZoneWithName:@"GMT"]];
    ...
    if (DateType == DateSelectedMonth) {
        ...
        if (chartType == BalanceChart) {
            ...
            for (int i = 0; i<dateMuArr.count; i++) {
                NSDate* sdate = dateMuArr[i];
                double amount = [self backgroundGetMonthNetworthWithDate:sdate account:account];
...
//            double num = [self getChartAmountMax:account];
        ...
        if (chartType == BalanceChart) {
            ...
            for (int i = 0; i < dateMuArr.count; i++) {
                NSDate* sDate = dateMuArr[i];
                double amount = [self backgroundGetWeekNetworthWithDate:sDate account:account];
                if (amount != 0) {
                    hasData = YES;
                }
                startBalance += amount;
...
//    [calendar setTimeZone:[NSTimeZone timeZoneWithName:@"GMT"]];
    ...
    for (int i = 0; i < categoryArr.count; i++) {
        ...
        NSArray* array = [transArr filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"category = %@",category]];
...
+(NSArray*)getMonthWithStartData:(NSDate*)startDate endDate:(NSDate*)endDate account:(Accounts*)account{
    ...
    if (account) {
        NSPredicate * pre = [NSPredicate predicateWithFormat:@"incomeAccount = %@ or expenseAccount = %@",account,account];
...
+(NSArray*)getTransactionByStartDate:(NSDate*)startDate endDate:(NSDate*)endDate category:(Category*)category account:(Accounts*)account{
    NSDictionary *subs = [NSDictionary dictionaryWithObjectsAndKeys:startDate,@"startDate",endDate,@"endDate",category,@"category", nil];
    
    NSFetchRequest *fetchRequest = [[XDDataManager shareManager].managedObjectModel fetchRequestFromTemplateWithName:@"fetchTransactionByDateandCategory" substitutionVariables:subs];
    NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"dateTime" ascending:YES];
    NSMutableArray   *sortDescriptors = [[NSMutableArray alloc] initWithObjects:&sortDescriptor count:1];
    [fetchRequest setSortDescriptors:sortDescriptors];
    NSError *error =nil;
...
+(double)getBeforeAmountWithDate:(NSDate*)date startNetworth:(double)startBalance account:(Accounts*)account{
    ...
    if (account) {
        NSArray* array = [[XDDataManager shareManager]getObjectsFromTable:@"Transaction" predicate:[NSPredicate predicateWithFormat:@"state = %@ and dateTime <= %@ and (expenseAccount = %@ or incomeAccount = %@)",@"1",date,account,account] sortDescriptors:nil];
...
+(double)getChartAmountMax:(Accounts*)account{
    ...
    if (account) {
        ...
        NSArray* tranArr = [[XDDataManager shareManager]getObjectsFromTable:@"Transaction" predicate:[NSPredicate predicateWithFormat:@"state = %@ and (expenseAccount = %@ or incomeAccount = %@)",@"1",account,account] sortDescriptors:nil];
...
+(double)getAmountWithStartDate:(NSDate*)date account:(Accounts*)account{
    ...
    if (account) {
        ...
        NSArray* tranArr = [[XDDataManager shareManager]getObjectsFromTable:@"Transaction" predicate:[NSPredicate predicateWithFormat:@"state = %@ and dateTime <= %@ and (expenseAccount = %@ or incomeAccount = %@)",@"1",date,account,account] sortDescriptors:nil];
...
+(double)backgroundGetAmountWithStartDate:(NSDate*)date account:(Accounts*)account{
    ...
    if (account) {
        ...
        NSArray* tranArr = [[XDDataManager shareManager]backgroundGetObjectsFromTable:@"Transaction" predicate:[NSPredicate predicateWithFormat:@"state = %@ and dateTime < %@ and (expenseAccount = %@ or incomeAccount = %@)",@"1",date,account,account] sortDescriptors:nil];
...
+(double)backgroundGetMonthNetworthWithDate:(NSDate*)date account:(Accounts*)account{
    NSArray* array = [[XDDataManager shareManager] backgroundGetTransactionDate:date withAccount:account];
    double balance = 0;
...
+(double)backgroundGetWeekNetworthWithDate:(NSDate*)date account:(Accounts*)account{
    NSArray* array = [[XDDataManager shareManager] backgroundGetTransactionDate:date withAccount:account];
    double balance = 0;
