...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==2)
    {
        if ([formerPasscodeStyle isEqualToString:@"none"] ||formerPasscodeStyle == nil)
        {
            ...
            {
                //可以使用TouchID
                [context evaluatePolicy:kLAPolicyDeviceOwnerAuthentication localizedReason:NSLocalizedString(@"VC_OpenTouchID", nil) reply:^(BOOL success, NSError * _Nullable error) {
                    if (success)
                    {
