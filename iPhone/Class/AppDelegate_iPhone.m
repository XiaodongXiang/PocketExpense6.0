...
//#ifdef LITE
//        settings.others1 =@"LIMIT_FUNCTION";
//#endif
...
//    [Crashlytics startWithAPIKey:@"5396d094a6603764683087a0e2248c3bf6260141"];
    ...
    if ([PFUser currentUser])
    {
        ...
        NSData *avatarData=[avatarFile getData:&error];
        if (error)
        {
            NSLog(@"头像下载错误 %@",error);
        }
...
//        [_drawerVC setCloseDrawerGestureModeMask:MMCloseDrawerGestureModeAll];
    ...
    if ([self.settings.passcodeStyle isEqualToString:@"touchid"])
    {
        ...
        [context evaluatePolicy:kLAPolicyDeviceOwnerAuthentication localizedReason:@"Touch ID" reply:^(BOOL success, NSError * _Nullable error) {
            if (success)
            {
...
-(IBAction)adsBtnAction:(id)sender
{
