...
//    NSLog(@"mainPath:%@",mainPath);
    ...
    NSFetchRequest *fetchTransferTransaction = [[NSFetchRequest alloc]initWithEntityName:@"Transaction"];
    NSPredicate *pre = [NSPredicate predicateWithFormat:@"expenseAccount!=nil && incomeAccount !=nil"];
    [fetchTransferTransaction setPredicate:pre];
    NSError *error = nil;
    NSArray *objects = [[NSArray alloc]initWithArray:[self.managedObjectContext executeFetchRequest:fetchTransferTransaction error:&error]];
    
    for (int i=0; i<[objects count]; i++)
    {
        Transaction *oneTrans = [objects objectAtIndex:i];
        if (oneTrans.category != nil)
        {
            oneTrans.category = nil;
...
-(void)updateTheBarTitleWithPrice:(double )p withLocal:(NSLocale*)l
{
    ...
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setValue:[numberFmt stringFromNumber:[NSNumber numberWithDouble:p]] forKey:PURCHASE_PRICE];
    [userDefaults synchronize];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator 
{
	...
	NSString *storePath = [[self applicationDocumentsDirectory] stringByAppendingPathComponent:@"Expense5.0.0.sqlite"];
	
	NSFileManager *fileManager = [NSFileManager defaultManager];
	// If the expected store doesn't exist, copy the default store.
	if (![fileManager fileExistsAtPath:storePath]) 
	{
		NSString *defaultStorePath = [[NSBundle mainBundle] pathForResource:@"Expense5.0.0" ofType:@"sqlite"];
		if (defaultStorePath) 
		{
...
-(void)restoreDelete
{
    ...
    for (int i=0; i<dbName.count; i++)
    {
        ...
        NSError *error=nil;
        NSArray *array=[self downloadEveryData:query with:&error];
