...
-(void)getOldDataBaseInsertToNewDataBase_isBackup:(BOOL)isBackup{
    ...
    if([fileManager fileExistsAtPath:oldPath2])
	{
...
#else
        //////transaction
        int periodNum_transaction = 0;
        char*  sqlPeriod_transaction = nil;
        if (![versionNumber isEqualToString:@"4.5.1"]) {
            sqlPeriod_transaction="SELECT Z_PK,Z_ENT,Z_OPT,ZISCLEAR,ZTYPE,ZCATEGORY,ZEXPENSEACCOUNT,ZINCOMEACCOUNT,ZPARTRANSACTION,ZPAYEE,ZAMOUNT,ZDATETIME,ZNOTES,ZPHOTONAME,ZTRANSACTIONTYPE FROM ZTRANSACTION";
            ...
            if(sqlite3_prepare_v2(oldDatabase, sqlPeriod_transaction, -1, &statement, NULL)==SQLITE_OK)
            {
                while(sqlite3_step(statement)==SQLITE_ROW)
                {
                    ...
                    const char* insert_sql = "INSERT INTO ZTRANSACTION (Z_PK,Z_ENT,Z_OPT,ZISCLEAR,ZTYPE,ZCATEGORY,ZEXPENSEACCOUNT,ZINCOMEACCOUNT,ZPARTRANSACTION,ZPAYEE,ZAMOUNT,ZDATETIME,ZNOTES,ZPHOTONAME,ZTRANSACTIONTYPE) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            sqlPeriod_transaction="SELECT Z_PK,Z_ENT,Z_OPT,ZISCLEAR,ZRECURRINGTYPE,ZCATEGORY,ZEXPENSEACCOUNT,ZINCOMEACCOUNT,ZPARTRANSACTION,ZPAYEE,ZAMOUNT,ZDATETIME,ZNOTES,ZPHOTONAME,ZTRANSACTIONTYPE FROM ZTRANSACTION";
            //4.5.1之前的
            int result = sqlite3_prepare_v2(oldDatabase, sqlPeriod_transaction, -1, &statement, NULL);
            if(result==SQLITE_OK)
            {
                while(sqlite3_step(statement)==SQLITE_ROW)
                {
                    ...
                    const char* insert_sql = "INSERT INTO ZTRANSACTION (Z_PK,Z_ENT,Z_OPT,ZISCLEAR,ZRECURRINGTYPE,ZCATEGORY,ZEXPENSEACCOUNT,ZINCOMEACCOUNT,ZPARTRANSACTION,ZPAYEE,ZAMOUNT,ZDATETIME,ZNOTES,ZPHOTONAME,ZTRANSACTIONTYPE) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		if(periodNum_transaction!=0)
		{
            ...
            sqlite3_stmt *insert_statement=nil;
			result = sqlite3_prepare_v2(curDatabase, sqlUpdateFeed, -1, &insert_statement, NULL);
			result = sqlite3_bind_int(insert_statement,1,periodNum_transaction);
			result = sqlite3_step(insert_statement);
			result = sqlite3_finalize(insert_statement);
		}
		result=sqlite3_finalize(statement);
